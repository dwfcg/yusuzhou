<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:75:"/www/wwwroot/yusuzhou.youacloud.com/application/shop/view/order/orders.html";i:1511945544;}*/ ?>
<!doctype html>
    ...
    <body style="">
        ...
        <div id='container' >
            ...
            <div class="order_topbar">
                <div class="nav <?php if($status == ''): ?>on <?php endif; ?>" data-status=""  onclick="location.href = '<?php echo url('order/orders'); ?>'">全部</div>
                <div class="nav <?php if($status != '' && $status < 1): ?>on <?php endif; ?>" data-status="0" onclick="location.href = '<?php echo url('order/orders',array('status'=>0)); ?>'">待付款</div>
                <div class="nav <?php if($status == 1): ?>on <?php endif; ?>"  data-status="1" onclick="location.href = '<?php echo url('order/orders',array('status'=>1)); ?>'">待发货</div>
                <div class="nav <?php if($status == 2): ?>on <?php endif; ?>"  data-status="2" onclick="location.href = '<?php echo url('order/orders',array('status'=>2)); ?>'">待收货</div>
                <div class="nav <?php if($status == 3): ?>on <?php endif; ?>"  data-status="3" onclick="location.href = '<?php echo url('order/orders',array('status'=>3)); ?>'">已完成</div>
            </div>
          ...
          <?php if(!$orders): ?>
            ...
            <div class="order_main" data-orderid="111">
                <div class="title">订单号：<?php echo $v['order_sn']; ?><span><?php echo $order_status[$v['status']]; ?></span></div>

                <div class="good">
                    <div class="img"  onclick="location.href = '<?php echo url('book/detail',array('id'=>$v['goods_id'])); ?>'"><img src="<?php echo $v['pic']; ?>"/></div>
                    <div class='info' onclick="location.href = '<?php echo url('order/detail',array('id'=>$v['id'])); ?>'">
                        <div class='inner'>
                    ...
                    <div class="price">
                        <div class='pnum'><span class='marketprice'>￥<?php echo $v['price']; ?></span></div>
                        <div class='pnum'><span class='total'>×1</span></div>
                    </div>
                </div>
             
                <div class="info1">共 1 件商品&nbsp;实付：<span>￥<?php echo $v['price']; ?></span></div>
                <div class="sub">
                    <?php if($v['status'] == 0): ?>
                    ...
                    <div class="sub2 order_cancel" style='position:relative;width:56px;'>
                        <span style='position:absolute;display:block;width:56px;' class="ajax_btn" data-id="<?php echo $v['id']; ?>" data-status="-1">取消订单</span>
                    </div>
                    <?php endif; if($v['status'] == -1): ?>       
                    <div class="sub1 order_refund" class="ajax_btn" data-id="<?php echo $v['id']; ?>" data-status="-2">申请退款</div>
                    <?php endif; ?>-->
                    <?php if($v['status'] == 1): ?>
                    <div class="sub1 order_complete">待发货</div>
                    <?php endif; if($v['status'] == 2): ?>
                    <div class="sub1 order_complete" class="ajax_btn" data-id="<?php echo $v['id']; ?>" data-status="3">确认收货</div>
