...
class Order extends Common
{
    ...
    {
        parent::_initialize();
        $order_status = array('0'=>'待付款','1'=>'定制中','2'=>'待收货','3'=>'待评价','4'=>'已完成','-1'=>'已取消','-3'=>'已删除');
    ...
    //新版更新
        public function save()
        {
            ...
            //取消订单
            if ($status == -1) {
                $res = Db::name('collect_order')->where('id',$oid)->update(['status'=>$status]);
                show_api($res,'已取消');
            }else if ($status == 3) {
                //完成订单
                $res = Db::name('collect_order')->where('id',$oid)->update(['status'=>$status]);
                $data = Db::name('collect_order')->where('id',$oid)->select();
                show_api($data);
            }else if($status == -3){
                //删除订单
                $res = Db::name('collect_order')->where('id',$oid)->update(['status'=>$status]);
    ...
    //订单评价展示页
    public function collect_comment()
    {
        $id = input('oid');
        $thread = Db::name('collect_order')->find($id);
        // var_dump($thread);die;
...
//      }
    ...
    public function comment_add()
    {
        $data = input('post.');
//        dump($data);
        $orderInfo=Db::name('collect_order')->find($data['oid']);
        if(!$orderInfo)
        {
            return show_api('','',0);
        }
        $data['collect_id'] = input('post.oid');
        $data['goodsstatus'] = 1;
...
// var_dump($oid);die;
      if($status && $oid){
        $data['status'] = $status;
        $where['id'] = $oid;
        $result = Db::name('collect_order')->update($data,$where);
        if($result && $status == -1){
