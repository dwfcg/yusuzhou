...

{
    ...
    protected $table = '__COLLECT_ORDER__';
    ...
    public static function  getOrders($where,$start=0,$limilt=10){
        ...
        $count = Db::name('collect_order')->where($where)->count();
// var_dump($count);
        
         $orders = Db::name('collect_order')->where($where)->order('add_time desc')->select();
        // $orders = Db::name('shop_order')->where($where)->limit($start,$limilt)->select();
// var_dump($orders);die;
        foreach ($orders as &$row){

            $goods = Db::name('collect_good')->find($row['goods_id']);
...
$orders['count'] = $count;
    ...
    public static function  get_one($id){
       ...
       // 
        ...
        $detial = Db::name('collect_order')->find($id);

        $shop_goods = Db::name('collect_good')->find($detial['goods_id']);
