...
class Detail extends Common
{
        ...
        foreach ($comment as $key => &$va) {
               $comment[$key]['content'] = json_decode($va['content']);
                ...
                foreach($va['mos'] as $ke =>&$vs)
                {
                    ...
                    // $va['bei'] = Db::name('forum_reply')->alias('s')
                    //            ->join('user d','s.from_uid=d.id')
                    //            ->where(['to_uid'=>$va['mos'][$ke]['to_uid']])
                    //            ->field('s.id,s.comment_id,s.content as scontent,s.from_uid,s.to_uid,s.reply_status,d.name as dname')->select();
                    //     foreach ($va['bei'] as $aw =>&$lue) {
                    //         $va['bei'][$aw]['scontent']=json_decode($lue['scontent']);
                    //     }
                }
                // dump($vs);
          var_dump($comment);
  ...
  //转json
    ...
    public function guandong()
    {
        ...
        $guan = Db::name('user_guan')->alias('a')
                ...
                ->where(['dangid'=>$dangid,'a.status'=>0])
  ...
  //ceshi
    ...
    public function huifu()
    {
        ...
        Db::name('forum_comment')->where(['uid'=>$bei,'id'=>$uid])->setInc('hui_num');
    ...
    public function suoyou()
    {
        ...
        $data = Db::name('forum_reply')->alias('a')
                ->join('forum_comment b','a.comment_id=b.uid')
                ->join('user c','a.comment_id=c.id')
                ->where(['a.comment_id'=>$id])
         		// ->field('a.id,a.comment_id,a.from_uid,a.to_uid,a.content,a.reply_status,a.addtime,b.name')
                ->select();
        show_api($data);
    }
    //文章下面的所有评论
    ...
    {
        $tid = input('post.tid');
        $cha = Db::name('forum_comment')->where(['tid'=>$tid])->field('id,uid,images,content,uname')->select();
        foreach ($cha as $key => &$va) {
            $cha[$key]['content']=json_decode($va['content']);
            $va['hui'] = Db::name('forum_reply')->where(['commet_id'=>$va['id']])
