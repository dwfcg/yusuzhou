...
use app\shop\model\Kind as KindModel;
...

{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 数据列表
        $data_list = KindModel::where($map)->column(true);
        if (empty($map)) {
       ...
       // $data_list = Db::name('shop_category')->where($map)->order('sort asc')->paginate();
        ...
        return ZBuilder::make('table')
            ...
            ->setSearch(['name' => '白度名称']) // 设置搜索框

            ->addColumns([ // 批量添加数据列

                ['id', 'ID'],

                ['name', '白度名称', 'callback', function($value, $data){
    ...
    public function add($pid = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($advert = Db::name('shop_kind')->insert($data)) {
                $this->success('新增成功', 'index');
            } else {
                $this->error('新增失败');
            }
        }
        $lists =  KindModel::getTreeList();
        // 显示添加页面
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data = $this->request->post();
            $data['update_time'] = time();
            if ($advert = Db::name('shop_kind')->where('id',$data['id'])->update($data)) {
                $this->success('编辑成功', 'index');
            } else {
                $this->error('编辑失败');
            }
        }
        $info = Db::name('shop_kind')->find($id);
        // 显示编辑页面
        return ZBuilder::make('form')
            ->addFormItems([
                ['hidden', 'id'],
                ['select', 'pid', '所属分类', '<span class="text-danger">必选</span>', KindModel::getTreeList($id)],
                ['text', 'name', '名称'],
                ['radio', 'status', '状态', '', ['停用', '启用'], 1]
