...
use app\shop\model\Cate as CateModel;
...

{
    ...
    public function add($pid = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($advert = Db::name('shop_category')->insert($data)) {
        ...
        return ZBuilder::make('form')
            ->addFormItems([
                ...
                ['radio', 'tuijian', '是否推荐到前台','<span class="text-danger">只能推荐顶级分类为0的分类,并且推荐数为5或5的倍数</span>', ['不推荐', '推荐'], 0, ]
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data = $this->request->post();
            $data['update_time'] = time();
            if ($advert = Db::name('shop_category')->where('id',$data['id'])->update($data)) {
                $this->success('编辑成功', 'index');
            } else {
                $this->error('编辑失败');
            }
        }
        $info = Db::name('shop_category')->find($id);
        // 显示编辑页面
        return ZBuilder::make('form')
            ->addFormItems([
                ['hidden', 'id'],
