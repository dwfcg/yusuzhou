...
use app\shop\model\Cation as CationModel;
...

{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 数据列表
        $data_list = CationModel::where($map)->column(true);
        if (empty($map)) {
    ...
    public function add($pid = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($advert = Db::name('shop_cation')->insert($data)) {
                $this->success('新增成功', 'index');
            } else {
                $this->error('新增失败');
            }
        }
        $lists =  CationModel::getTreeList();
        // 显示添加页面
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data = $this->request->post();
            $data['update_time'] = time();
            if ($advert = Db::name('shop_cation')->where('id',$data['id'])->update($data)) {
                $this->success('编辑成功', 'index');
            } else {
                $this->error('编辑失败');
            }
        }
        $info = Db::name('shop_cation')->find($id);
        // 显示编辑页面
        return ZBuilder::make('form')
            ->addFormItems([
                ['hidden', 'id'],
                ['select', 'pid', '所属分类', '<span class="text-danger">必选</span>', CationModel::getTreeList($id)],
                ['text', 'name', '皮色名称'],
                ['radio', 'status', '状态', '', ['停用', '启用'], 1]
